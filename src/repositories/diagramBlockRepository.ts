...
export class DiagramBlockRepository {
  ...
  async update(id: string, input: DiagramBlockUpdateInput): Promise<DiagramBlock | null> {
    ...
    if (input.x !== undefined) { fields.push(`x = $${idx++}`); values.push(input.x); }
    if (input.y !== undefined) { fields.push(`y = $${idx++}`); values.push(input.y); }
    if (input.width !== undefined) { fields.push(`width = $${idx++}`); values.push(input.width); }
    if (input.height !== undefined) { fields.push(`height = $${idx++}`); values.push(input.height); }
    if (input.properties !== undefined) { fields.push(`properties = $${idx++}`); values.push(input.properties); }
