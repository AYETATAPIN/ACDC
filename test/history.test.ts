...
before(async () => {
  ...
  try {
    ...
    console.warn('Skipping history integration tests: database is not available.', error);
  }
});
...
test('undo/redo returns previous diagram states', async (t) => {
  if (!dbReady) {
...
test('undo after creating block clears blocks', async (t) => {
  if (!dbReady) {
    t.skip('Database is not available in this environment');
    return;
  }
